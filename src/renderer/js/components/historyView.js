...
function renderTable(tbody, colCount) {
  ...
  currentRows.forEach((row, idx) => {
    ...
    tr.innerHTML = '<td class="px-2 py-2 text-on-surface-variant" style="font-size:10px;">' + formatDate(row.fecha) + '</td>'
      + '<td class="px-2 py-2" style="font-size:10px;">' + clienteCell + '</td>'
      + '<td class="px-2 py-2 font-medium" style="font-size:10px;">' + formatDimensions(row.largo_cm, row.ancho_cm, row.alto_cm) + '</td>'
      + '<td class="px-2 py-2 font-medium" style="font-size:10px;">' + row.cp_prefix + '</td>'
      + '<td class="px-2 py-2" style="font-size:10px;">' + formatVolume(row.metros_cubicos) + '</td>'
      + '<td class="px-2 py-2 font-bold text-primary" style="font-size:10px;">' + (row.peso != null ? row.peso + ' kg' : '—') + '</td>'
      + '<td class="px-2 py-2" style="font-size:10px;">' + (row.agencia_nombre || '—') + '</td>'
      + '<td class="px-2 py-2 text-right"><span class="font-black text-primary" style="font-size:11px;">' + formatPrice(row.precio_final) + '</span></td>'
      + '<td class="px-2 py-2 text-right"><span class="font-bold" style="font-size:11px;color:#15803d;">' + formatPrice(redondeado) + '</span></td>'
      + '<td class="px-2 py-2 truncate" style="font-size:10px;">' + (row.destinatario || '<span style="opacity:0.3;">—</span>') + '</td>'
      + '<td class="px-1 py-2 text-center">'
      + '<div class="flex items-center justify-center gap-0">'
      + '<button class="btn-edit-row w-6 h-6 flex items-center justify-center rounded-md transition-colors hover:bg-blue-100" title="Editar" data-row-id="' + row.id + '">'
      + '<span class="material-symbols-outlined" style="font-size:14px; color:#0040e0;">edit</span>'
      + '</button>'
      + '<button class="btn-delete-row w-6 h-6 flex items-center justify-center rounded-md transition-colors hover:bg-red-100" title="Borrar" data-row-id="' + row.id + '">'
